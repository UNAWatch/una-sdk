...
This repository regenerates the checked-in C SDK with `../regenerate-c-sdk.ps1`.
That script preserves the existing `+Custom` suffix in generated `Profile Version`
headers, including files such as `fit_ram.c`, while running FitGen with
`-norewrite` so the existing `config.csv` is kept. When upgrading to a new
Garmin FIT profile or FitGen release, keep any explicit custom blocks in place,
run `regenerate-c-sdk.ps1`, and verify that generated `Profile Version` lines
still include the intended `+Custom` suffix before committing.
