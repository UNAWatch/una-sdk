...
{
    FIT_UINT16 left = 0;
    FIT_UINT16 right = FIT_MESGS - 1;

   while (left <= right)
   {
        FIT_UINT16 mid = (left + right) / 2;
      if (fit_base_mesg_defs[mid]->global_mesg_num == global_mesg_num) 
        return (FIT_MESG_DEF *) fit_base_mesg_defs[mid];
      else if (fit_base_mesg_defs[mid]->global_mesg_num < global_mesg_num)
         left = mid + 1;
      else
         right = mid - 1;
   }
