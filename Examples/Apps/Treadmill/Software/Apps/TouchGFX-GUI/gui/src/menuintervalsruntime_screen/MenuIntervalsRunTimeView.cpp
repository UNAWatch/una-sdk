...
MenuIntervalsRunTimeView::MenuIntervalsRunTimeView() :
    ...
    mUpdateCenterItemCb(this, &MenuIntervalsRunTimeView::updateCenterItem)
{
...
void MenuIntervalsRunTimeView::setupScreen()
{
    MenuIntervalsRunTimeViewBase::setupScreen();
...
void MenuIntervalsRunTimeView::tearDownScreen()
{
    MenuIntervalsRunTimeViewBase::tearDownScreen();
}

void MenuIntervalsRunTimeView::setTime(uint32_t totalSeconds)
{
...
void MenuIntervalsRunTimeView::enterStage(Stage stage)
{
    mStage = stage;
    if (stage == STAGE_MIN) {
        menuLayout.setNumberOfItems(Menu::kCountMin);
...
void MenuIntervalsRunTimeView::updateItem(MainMenuItem& item, int16_t index)
{
...
void MenuIntervalsRunTimeView::updateCenterItem(MainMenuCenterItem& item, int16_t index)
{
    uint16_t minutes, seconds;
    if (mStage == STAGE_MIN) {
        minutes = static_cast<uint16_t>(index) * Menu::kStepMin;
...
void MenuIntervalsRunTimeView::handleKeyEvent(uint8_t key)
{
    ...
    if (key == SDK::GUI::Button::R1) {
        if (mStage == STAGE_MIN) {
            ...
            presenter->saveRunTime(mMinutes * 60u + mSeconds);
