...
ActivityWriter::ActivityWriter(const SDK::Kernel& kernel, const char* pathToDir)
   ...
   , mFHRecord(skRecordMsgNum, (FIT_MESG_DEF*)fit_mesg_defs[FIT_MESG_RECORD])
    , mFHStrainField(skStrainMsgNum, 0, {&mFHRecord})
    , mFHActiveField(skActiveMsgNum, 1, {&mFHRecord})
{
    ...
    mFHStrainField.init({FIT_FIELD_DESCRIPTION_FIELD_NUM_FIELD_NAME,
    ...
    mFHActiveField.init({FIT_FIELD_DESCRIPTION_FIELD_NUM_FIELD_NAME,
                         FIT_FIELD_DESCRIPTION_FIELD_NUM_UNITS,
                         FIT_FIELD_DESCRIPTION_FIELD_NUM_DEVELOPER_DATA_INDEX,
                         FIT_FIELD_DESCRIPTION_FIELD_NUM_FIELD_DEFINITION_NUMBER,
                         FIT_FIELD_DESCRIPTION_FIELD_NUM_FIT_BASE_TYPE_ID});
...
void ActivityWriter::start(const AppInfo& info)
{
    ...
    {
        // Strain field desc
        mFHStrainField.writeDef(fp);

        FIT_FIELD_DESCRIPTION_MESG strainDesc{};

        strncpy(strainDesc.field_name, "strain", FIT_FIELD_DESCRIPTION_MESG_FIELD_NAME_COUNT);
        strncpy(strainDesc.units, "score", FIT_FIELD_DESCRIPTION_MESG_UNITS_COUNT);
        strainDesc.developer_data_index    = 0;
        strainDesc.field_definition_number = 0;
        strainDesc.fit_base_type_id        = FIT_BASE_TYPE_FLOAT32;

        mFHStrainField.writeMessage(&strainDesc, fp);
    }

    {
        // Active min field desc
        mFHActiveField.writeDef(fp);

        FIT_FIELD_DESCRIPTION_MESG activeDesc{};

        strncpy(activeDesc.field_name, "active_min", FIT_FIELD_DESCRIPTION_MESG_FIELD_NAME_COUNT);
        strncpy(activeDesc.units, "minutes", FIT_FIELD_DESCRIPTION_MESG_UNITS_COUNT);
        activeDesc.developer_data_index    = 0;
        activeDesc.field_definition_number = 1;
        activeDesc.fit_base_type_id        = FIT_BASE_TYPE_UINT32;

        mFHActiveField.writeMessage(&activeDesc, fp);
    }
...
void ActivityWriter::addRecord(const RecordData& record)
{
    ...
    if (record.has(RecordData::Field::HEART_RATE)) {
        msg.heart_rate = record.heartRate;
    }

    mFHRecord.writeMessage(&msg, fp);

    if (record.has(RecordData::Field::STRAIN)) {
        FIT_FLOAT32 strain = record.strain;
        mFHRecord.writeFieldMessage(0, &strain, fp);
    }

    if (record.has(RecordData::Field::ACTIVE_MIN)) {
        FIT_UINT32 active = record.activeMin;
        mFHRecord.writeFieldMessage(1, &active, fp);
    }
}
...
void ActivityWriter::addLap(const LapData& lap)
{
    ...
    lap_mesg.total_ascent = lap.totalAscent;
    lap_mesg.total_descent = lap.totalDescent;
...
void ActivityWriter::stop(const TrackData& track)
{
    ...
    // Write Session message.
    {
        ...
        session_mesg.total_ascent = track.totalAscent;
        session_mesg.total_descent = track.totalDescent;
...
bool ActivityWriter::createAndOpenFile(std::time_t utc)
{
    ...
    // Create file with strain naming
    snprintf(buff + len, sizeof(buff) - len, "strain_%04u-%02u-%02u.fit",
        localTime.tm_year + 1900, localTime.tm_mon + 1, localTime.tm_mday);

    mFile = mKernel.fs.file(buff);
    if (!mFile || !mFile->open(true, true)) { // write mode, create/override
