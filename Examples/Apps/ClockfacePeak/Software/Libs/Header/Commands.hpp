 ...
 ******************************************************************************
 * @file    Commands.hpp
 * @brief   Messages exchanged between the Peak clockface service and GUI.
 ******************************************************************************
 */
...
constexpr SDK::MessageType::Type GOALS        = 0x00000006;
 ...
 *
 * They come from the same system-settings read as the clock format and change
 * on the same terms -- rarely, and only from the phone -- so they travel the
 * same path rather than getting a schedule of their own. A zero target draws
 * an empty ring: there is no fraction to be a part of.
 */
struct Goals : public SDK::MessageBase {
    uint32_t steps;             ///< Target steps per day
    uint32_t activityMinutes;   ///< Target active minutes per day

    Goals()
        : SDK::MessageBase(GOALS)
        , steps(0)
        , activityMinutes(0)
    {}

    Goals(uint32_t steps, uint32_t activityMinutes)
        : Goals()
    {
        this->steps           = steps;
        this->activityMinutes = activityMinutes;
...
static_assert(sizeof(Goals) <= 256, "Goals must fit the largest kernel message pool");
