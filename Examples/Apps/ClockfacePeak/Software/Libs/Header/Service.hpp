 ...
 ******************************************************************************
 * @file    Service.hpp
 * @brief   Peak clockface service: the clock, the charge level and the day's
...
private:
    ...
    /** @brief Tell the GUI the daily targets the rim arcs are fractions of. */
    void publishGoals();
    ...
    uint32_t mActivityGoal;
    uint32_t mSentStepsGoal;        ///< Last targets sent to the GUI
    uint32_t mSentActivityGoal;
    bool     mGoalsSent;            ///< Targets have reached the GUI
