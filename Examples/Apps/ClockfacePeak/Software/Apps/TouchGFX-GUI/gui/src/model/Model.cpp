...
bool Model::customMessageHandler(SDK::MessageBase *message)
{
    ...
    switch (message->getType()) {
        ...
        case CustomMessage::GOALS: {
            auto *msg = static_cast<CustomMessage::Goals*>(message);
            const DailyGoals goals{ msg->steps, msg->activityMinutes };
            if (!(mGoals == goals)) {
                mGoals = goals;
                if (modelListener) {
                    modelListener->onGoals(mGoals);
                }
            }
        } break;
