#!/usr/bin/env python3
"""
make_icon_header.py — Convert RGBA PNG(s) to ABGR2222 byte arrays in a C header.

- Rotates each image 90° (as per your original function).
- Requires square PNGs (width == height).
- Emits one header with arrays + WIDTH/HEIGHT/SIZE macros for each icon.

Usage:
  python make_icon_header.py icon.png
  python make_icon_header.py icon1.png icon2.png -o icons.h
  python make_icon_header.py battery.png -o battery_icon.h --name BATTERY
