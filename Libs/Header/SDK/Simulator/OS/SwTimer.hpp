...
public:
    ...
    SwTimer(uint32_t interval)           {m_Active = true;  m_Complete = false; m_Interval = interval;   m_PrevSysTimer = getTicks(); };
    ...
    void start(void)                     { m_PrevSysTimer = getTicks(); m_Active = true; }
    void start(uint32_t value)           { setInterval(value); start(); }

    void stop(void)                      { setActive(false); }

    void reset(void)                     { m_PrevSysTimer = getTicks(); }
    ...
    bool expired(void)
    {
        ...
        return (passed(getTicks(), m_PrevSysTimer) >= m_Interval);
    };
    ...
    uint32_t left(void)
    {
        ...
        uint32_t passedMS = passed(getTicks(), m_PrevSysTimer);
